...
const ProductReviews = (productId) => {
    getProducts()
        ...
        <dialog open class="reviewDialog">
        <section class="reviewContent">
          <div><button id="reviewClose">x</button></div>
          <div>
          <h4>Reviews for: ${reviewedProduct.name} ${categories.name}</h4>
          <div>${filterReviews.map(review => `<div class="review">${review.date}<br>Rating: ${review.rating}<br>Message: ${review.message}</div>`).join("")}</div>
          </div>
        <section>
        </dialog>
        `
